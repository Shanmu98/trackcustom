...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String hotel = getIntent().getExtras().getString("hotel");
        //final String user1 = getIntent().getExtras().getString("name1");
        Firebase fm = new Firebase("https://foodtrack-1afcd.firebaseio.com/hotels/"+hotel+"/customers/"+user+"/location");
        fm.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                try {
                    List<Address> address=gc.getFromLocation(lat,lon,1);
                ...
                if(mark!=null)
                {
                    cur=ll;
                    mark.remove();
                    mMap.addMarker(new MarkerOptions().position(prev).title(user+ " recorded location").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_BLUE)).snippet(addr));
                ...
                addr=st;
                prev=ll;
                mark=mMap.addMarker(new MarkerOptions().position(ll).title(user + " current location"+st).icon(BitmapDescriptorFactory.fromResource(R.mipmap.car)).snippet(st));
